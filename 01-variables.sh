#!/bin/bash

PERSON1=$1
PERSON2=$2

echo "$PERSON1:: HI $PERSON2"
echo "$PERSON2:: hello $PERSON1"
echo "$PERSON1:: how are you $PERSON2"
echo "$PERSON2:: fine and you"
echo "$PERSON1:: iam also fine how was the preparartion $PERSON2"
echo "$PERSON2:: good its going on and how about you"
echo "$PERSON1:: im also trying my best $PERSON2"
echo "$PERSON2:: ohh ok $PERSON1"
echo "$PERSON1:: ok ALL THE BEST AND BEST OF LUCK! $PERSON2"
echo "$PERSON2:: same to you $PERSON1"
